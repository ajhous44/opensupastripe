    ...
    <motion.div ref={ref} className={cn('relative mx-auto h-full w-full', className)}>
      <div className="absolute top-3 -left-4 md:-left-20">
        <motion.div
          ...
          className="ml-[27px] flex size-4 items-center justify-center rounded-full border border-blue-500/30 shadow-sm"
        >
          <motion.div
            transition={{ duration: 0.2, delay: 0.5 }}
            animate={{
            ...
            className="size-2 rounded-full border border-blue-500 bg-white"
          />
        </motion.div>
        <svg viewBox={`0 0 20 ${svgHeight}`} width="20" height={svgHeight} className="ml-4 block" aria-hidden>
          ...
          <motion.path
            d={`M 1 0V -36 l 18 24 V ${svgHeight * 0.8} l -18 24V ${svgHeight}`}
            fill="none"
            stroke="url(#supastripe-tracing-gradient)"
            strokeWidth="1.25"
          ...
          <defs>
            <motion.linearGradient
              id="supastripe-tracing-gradient"
